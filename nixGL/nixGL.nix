...
, fetchurl, lib, runtimeShell, bumblebee, libglvnd, spirv-tools, vulkan-validation-layers
, mesa, libvdpau-va-gl, intel-media-driver#, pkgsi686Linux, driversi686Linux
, zlib, libdrm, xorg, wayland, gcc, zstd }:

let
  vulkan-validation-layers-patched = vulkan-validation-layers.overrideAttrs (prev: {
    buildInputs = prev.buildInputs ++ [spirv-tools];
  });
      ...
      nixNvidiaWrapper = api:
        writeExecutable {
          ...
          text = ''
            ...
            "export VK_LAYER_PATH=${vulkan-validation-layers-patched}/share/vulkan/explicit_layer.d"}
            ...
            ${''export __EGL_VENDOR_LIBRARY_FILENAMES=''${NVIDIA_JSON[*]}${
              ...
              export LD_LIBRARY_PATH=${
                lib.makeLibraryPath ([ libglvnd nvidiaLibsOnly ]
                  ++ lib.optional (api == "Vulkan") vulkan-validation-layers-patched
                  ++ lib.optionals enable32bits [
